import{j as e,r as $}from"./index-BrsPDACf.js";const ve=["٠","١","٢","٣","٤","٥","٦","٧","٨","٩"],a=(t,n)=>{const r=String(t);return n?r.replace(/[0-9]/g,o=>ve[parseInt(o)]):r},Z=(t,n)=>{try{return new Intl.DateTimeFormat(t,n)}catch{return null}},Q=Z("ar-SA-u-nu-latn",{day:"2-digit",month:"2-digit",year:"numeric",calendar:"gregory"}),ee=Z("ar-SA-u-ca-islamic-umalqura-nu-latn",{day:"2-digit",month:"2-digit",year:"numeric",calendar:"islamic-umalqura"}),ke=t=>ee?ee.format(t).replace(" هـ",""):"N/A",te=Z("ar-SA",{weekday:"long"}),Y=t=>{if(Q)return Q.format(t);const n=String(t.getDate()).padStart(2,"0"),r=String(t.getMonth()+1).padStart(2,"0"),o=t.getFullYear();return`${n}/${r}/${o}`},J=t=>ke(t),se=t=>te?te.format(t):["الأحد","الاثنين","الثلاثاء","الأربعاء","الخميس","الجمعة","السبت"][t.getDay()],re=t=>{if(!t)return null;const n=new Date(t);return isNaN(n.getTime())?null:n},ne=(t,n)=>{const r=re(t),o=re(n);if(!r||!o||o<r)return[];const l=[];let u=new Date(r.getTime());for(;u<=o;)l.push({gregorianDate:new Date(u.getTime()),topic:""}),u.setDate(u.getDate()+1);return l},Ne=t=>{if(!t.days.length){alert("لا يوجد جدول لتصديره.");return}const n={version:5,...t,days:t.days.map((l,u)=>({index:u+1,topic:l.topic,gregorianDate:l.gregorianDate.toISOString(),isSpecial:l.isSpecial||!1}))},r=new Blob([JSON.stringify(n,null,2)],{type:"application/json;charset=utf-8;"}),o=`plan_${t.startDate}_to_${t.endDate}.json`;U(r,o)},De=t=>{if(!t.length){alert("لا يوجد جدول لتصديره.");return}const n=t.map(o=>o.topic.trim().replace(/\r?\n/g," ")).filter(Boolean).join(`\r
`);if(!n){alert("لا توجد موضوعات مكتوبة لتصديرها.");return}const r=new Blob(["\uFEFF"+n],{type:"text/plain;charset=utf-8;"});U(r,"topics.txt")},U=(t,n)=>{const r=URL.createObjectURL(t),o=document.createElement("a");o.href=r,o.download=n,document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(r)},ae=()=>`
...
`,Ce=(t,n)=>{const{useEasternDigits:r}=t;return`
    ...
    `:""},Te=(t,n,r)=>{const o=a(J(t),r.useEasternDigits),l=a(J(n),r.useEasternDigits),u=a(Y(t),r.useEasternDigits),k=a(Y(n),r.useEasternDigits);return`(${o} - ${l}هـ | ${u} - ${k}م)`},$e=(t,n)=>{const r=a(J(t),n.useEasternDigits),o=a(Y(t),n.useEasternDigits);return`(${r}هـ - ${o}م)`},Ee=t=>{const{days:n,meta:r,viewMode:o,title:l}=t,u=Ce(r,l),k=We(r);let b="";const g=o==="table"?'<w:pgSz w:w="11906" w:h="16838"/>':'<w:pgSz w:w="16838" w:h="11906" w:orient="landscape"/>',f='<w:pgMar w:top="720" w:right="720" w:bottom="720" w:left="720" w:header="708" w:footer="708" w:gutter="0"/>';b=`
        ...
        `;const m=o==="table"?"21cm 29.7cm":"29.7cm 21cm";let c="";if(o==="table"){let W="";const L=r.hiddenWeeks||[];let T=-1,y=0;const I=new Map;for(let i=0;i<n.length;i++){const v=n[i].gregorianDate.getDay(),E=i===0||v===0;if(E&&T++,!I.has(i)){let C=1;if(E){for(let j=i+1;j<n.length&&n[j].gregorianDate.getDay()!==0;j++)C++;L.includes(T)||y++}I.set(i,{num:y,span:C,processed:!1})}}T=-1,n.forEach((i,p)=>{const v=i.gregorianDate.getDay(),E=p===0||v===0;E&&T++;const C=v===5||v===6,j=r.hideWeekends&&C;if(L.includes(T))return;let R="transparent";i.isSpecial?R="#fefcbf":j&&(R="#e2e8f0");let O="";if(E){const z=I.get(p);z&&(O=`<td rowspan="${z.span}" style="border:1px solid black; background-color:transparent; text-align:center; vertical-align:middle; font-weight:bold;">${a(z.num,r.useEasternDigits)}</td>`)}const N=$e(i.gregorianDate,r),P=(i.topic||"").replace(/\n/g,"<br>");W+=`
                <tr style="background-color: ${R};">
                    ...
                    <td style="border:1px solid black; text-align:center; vertical-align:middle; white-space:nowrap;">${se(i.gregorianDate)}</td>
        ...
        `}else{const W=r.hiddenWeeks||[],L=n.reduce((i,p,v)=>((p.gregorianDate.getDay()===0||v===0)&&i.push([]),i[i.length-1].push(p),i),[]);let T=0,y=[];const I=[];if(L.forEach((i,p)=>{if(W.includes(p))return;T++;const E=i[0].gregorianDate,C=i[i.length-1].gregorianDate,j=i.filter(N=>{const P=N.gregorianDate.getDay(),z=P===5||P===6;return!(r.hideWeekends&&z)&&N.topic&&N.topic.trim()!==""&&N.topic.trim()!=="--- عطلة نهاية الأسبوع ---"&&N.topic.trim()!=="--- إجازة رسمية ---"});let A="";j.length>0?A=`<div style="margin: 0; padding-right: 5px; margin-top:2px;">
                    ${j.map((N,P)=>`
    ...
    `,F=new Blob(["\uFEFF",w],{type:"application/msword"});U(F,`plan_export_${new Date().getTime()}.doc`)},Se=async t=>{const n=document.getElementById("printable-area-content"),{meta:r,title:o}=t,{useEasternDigits:l}=r;if(!n){alert("حدث خطأ: لا يمكن العثور على المحتوى للتصدير.");return}const u=`
  ...
  `,f=n.cloneNode(!0);f.querySelectorAll("textarea[data-row-index]").forEach(T=>{var v;const y=T,I=document.createElement("div");I.innerHTML=y.value.replace(/\n/g,"<br />");const i=y.closest("tr"),p=i&&i.classList.contains("print:hidden");I.className=`p-2 w-full h-full ${y.disabled?"text-center":""}`,(y.disabled||p)&&I.classList.add("bg-gray-200"),(v=y.parentNode)==null||v.replaceChild(I,y)});const m=Array.from(document.querySelectorAll("style")).map(T=>T.innerText).join(`
